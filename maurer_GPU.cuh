...
// texConstSrc - contains the constant input image
// extern texture<uchar>  texConstSrc;
// texIn and texOut are ping-ponged between operatins
texture<int>  ping_pong_tex_0;
texture<int>  ping_pong_tex_1;
...
template <typename T>
__global__ void raster_scan_GPU_v2(T *input, int round, int height, int width, int depth, \
									int *dev_output)
{
	...
	*/
	...
	while (thread_pos[round] < vol_dim[round])
	{
		...
		int partial_cfv_valid = 0; // This indicate that contents in partial_cfv is currently valid or not
